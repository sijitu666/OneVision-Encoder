...
```bash
tools/build_subtitle_event_html_report.py
```

用途：

- 把 `events.json` 中的字幕切换候选做成静态 HTML。
- 每个事件都展示 candidate 前后 5 帧窗口。
- 同时生成字幕 ROI 放大图、前后帧差分热力图、全画面上下文和 H.264 `i/I` 宏块 overlay 对照。
- 支持按 event id、frame id、onset/offset 类型搜索和过滤。

典型用法：

```bash
/opt/anaconda3/bin/python tools/build_subtitle_event_html_report.py \
  --event-json outputs/subtitle_codec_events/duanju_ocrbox_codec/events.json \
  --codec-overlay-frames outputs/h264_mbtype_intra_overlays/duanju/frames \
  --out-dir outputs/subtitle_codec_events/duanju_ocrbox_codec/html_report \
  --window 2 \
  --context-pad-y 100 \
  --full-height 420 \
  --crop-width 520
...
- `html_report/index.html`：主报告页面。
- `html_report/assets/`：每个事件的 full/crop/diff/codec 图片。
- `html_report/report_manifest.json`：报告输入、ROI 和 overlay 目录记录。
